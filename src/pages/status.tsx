...
const getStatusProps = (status: Status) => {
    switch (status) {
        case 'Operational':
            return { color: 'text-indigo-400', Icon: CheckCircle2, label: 'Operational' };
        ...
        case 'Under Maintenance':
            return { color: 'text-indigo-400', Icon: Wrench, label: 'Under Maintenance' };
        default:
        ...
        <div className="min-h-screen text-white" style={{ backgroundImage: `url('/background.png')`, backgroundAttachment: 'fixed', backgroundSize: 'cover' }}>
            <div className="container mx-auto px-4 py-20 pt-32">
                ...
                <motion.div initial={{ opacity: 0, y: -30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }} className="text-center mb-12">
                    <BarChartHorizontal className="w-16 h-16 text-indigo-400 mx-auto mb-4" />
                ...
                <motion.div
                    ...
                    className={`max-w-4xl mx-auto p-4 rounded-lg mb-12 border ${overallStatus.includes('All') ? 'bg-indigo-500/10 border-indigo-400' : 'bg-yellow-500/10 border-yellow-400'}`}
                >
                    <p className={`font-bold text-center ${overallStatus.includes('All') ? 'text-indigo-300' : 'text-yellow-300'}`}>{overallStatus}</p>
                </motion.div>
                        ...
                        {incidentHistory.map((incident, index) => (
                            ...
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div>
                                        <h3 className="font-bold text-xl text-indigo-300">{incident.title}</h3>
                                        <p className="text-sm text-gray-400">{incident.date}</p>
                                    </div>
                                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${incident.status === 'Resolved' ? 'bg-indigo-500/20 text-indigo-300' : 'bg-yellow-500/20 text-yellow-300'}`}>
                                        {incident.status}
                                    </span>
                                </div>
